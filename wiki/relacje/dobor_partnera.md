# Dobór partnera

## Wprowadzenie

Pytanie o to, dlaczego wybieramy właśnie tę, a nie inną osobę na partnera życiowego, należy do kluczowych zagadnień psychologii relacji. Odpowiedzi poszukuje się na trzech poziomach analizy: **ewolucyjnym** (jakie mechanizmy selekcji zostały ukształtowane przez dobór naturalny i płciowy), **psychologicznym** (jakie poznawcze i emocjonalne procesy regulują wybór) oraz **socjokulturowym** (jak normy społeczne i kontekst kulturowy filtrują preferencje). Żaden z tych poziomów nie wyczerpuje zjawiska — pełne zrozumienie wymaga ich integracji.

---

## Ewolucyjne teorie doboru partnera

### Teoria inwestycji rodzicielskiej — Trivers

Robert Trivers (1972) postulował, że **płeć inwestująca więcej w potomstwo** (u ludzi — kobiety, ze względu na ciążę, poród i laktację) jest bardziej selektywna w wyborze partnera, ponieważ błąd reprodukcyjny jest dla niej kosztowniejszy. Płeć inwestująca mniej (mężczyźni) rywalizuje o dostęp do płci bardziej inwestującej. Teoria ta przewiduje asymetrie płciowe w preferencjach partnerskich.

### Badania Bussa — studium 37 kultur

David Buss i współpracownicy (1989) przeprowadzili przełomowe badanie w **37 kulturach** na 6 kontynentach (N > 10 000). Wyniki:

| Cecha preferowana | Kobiety | Mężczyźni | Interpretacja ewolucyjna |
|---|:---:|:---:|---|
| **Status i zasoby** | ↑↑ ważna | mniej ważna | kobiety cenią zdolność zapewnienia zasobów |
| **Ambicja i pracowitość** | ↑↑ ważna | mniej ważna | wskaźnik przyszłych zasobów |
| **Atrakcyjność fizyczna** | mniej ważna | ↑↑ ważna | wskaźnik zdrowia i płodności |
| **Młodość partnerki** | neutralna | ↑↑ ważna | wyznacznik wartości reprodukcyjnej |
| **Inteligencja** | ↑↑ ważna | ↑↑ ważna | adaptacja ogólna, zasoby i geny |
| **Dobroć i łagodność** | ↑↑ ważna | ↑↑ ważna | wskaźnik kooperacyjności |

Różnice płciowe były **konsekwentne we wszystkich kulturach**, co stanowi argument za ich ewolucyjnym podłożem.

### Atrakcyjność fizyczna i symetria

Symetria twarzy i ciała jest wskaźnikiem **jakości genetycznej** i odporności na pasożyty w trakcie rozwoju. Badania Gangstada i Simpsona (2000) wykazały, że mężczyźni o wyższej symetrii ciała mają więcej partnerek seksualnych i wyższy status. Badania waist-to-hip ratio (WHR) Singha (1993) wskazują, że mężczyźni preferują kobiety z WHR ~0,7, co koreluje z optymalnym profilem hormonalnym.

---

## Dobór assortatywny

**Dobór assortatywny** (*assortative mating*) to tendencja do wybierania partnerów podobnych do siebie. Badania dokumentują podobieństwo par w zakresie:

- wieku (korelacja r ≈ 0,84)
- wykształcenia (r ≈ 0,60)
- inteligencji (r ≈ 0,40)
- postaw i wartości politycznych (r ≈ 0,65)
- cech osobowości (r ≈ 0,15–0,25; niższe niż zakładano)
- fizycznych cech (wzrost, waga, atrakcyjność) (r ≈ 0,20–0,40)

Dobór assortatywny może wynikać zarówno z **preferencji** (wybieramy podobnych), jak i z **proximity** (spotykamy podobnych w tych samych środowiskach edukacyjnych i zawodowych).

---

## Hipoteza podobieństwa i atrakcji

Donn Byrne (1971) sformułował **hipotezę podobieństwa-atrakcji**: lubienie wzrasta proporcjonalnie do proporcji zgodnych postaw. Mechanizmy:

1. **Walidacja społeczna** — osoba podobna potwierdza nasze przekonania i wzmacnia poczucie własnej wartości
2. **Antycypacja harmonii** — przewidujemy, że będziemy lepiej rozumiani przez osobę podobną
3. **Efekt znajomości** (*mere exposure*, Zajonc 1968) — częsty kontakt z podobnymi zwiększa ich atrakcyjność

**Zastrzeżenia:** Efekt podobieństwa jest silniejszy dla postaw niż dla cech osobowości; w długoterminowych związkach uzupełnienie (*complementarity*) może być ważniejsze w niektórych domenach (np. dominancja-submisja).

---

## Wpływy socjokulturowe

### Pole dostępnych partnerów

Kluczowym socjologicznym pojęciem jest **pole dostępnych partnerów** (*field of eligibles*, Kerckhoff, 1974) — społecznie określona pula osób, z którymi kontakt jest możliwy. Determinują je:
- endogamia kulturowa i religijna
- homogamia klasowa (spotykamy się z osobami z podobnych środowisk)
- geografia i przestrzeń życiowa

### Teorie filtrowania

Teorie sekwencyjnego filtrowania (Murstein 1970 — **SVR model**: Stimulus–Value–Role) zakładają, że dobór przebiega etapami:
1. **Stimulus** — ocena atrakcyjności fizycznej i społecznej
2. **Value** — porównanie wartości, postaw, religii
3. **Role** — kompatybilność ról (praca, rodzicielstwo, finanse)

---

## Psychologia randkowania online

Internet fundamentalnie zmienił krajobraz doboru partnera. W Polsce do 2023 r. ok. 30% nowych związków zawiązało się przez aplikacje randkowe (*CBOS, 2022*). Badania wskazują:

| Aspekt | Efekt aplikacji randkowych |
|---|---|
| **Pole dostępnych partnerów** | drastyczne poszerzenie — tysiące profili w zasięgu |
| **Primacy efekt atrakcyjności** | zwiększona rola wyglądu (selekcja na podstawie zdjęcia) |
| **Paradoks wyboru** | więcej opcji → niższa satysfakcja, wyższe wymagania (*Schwartz, 2004*) |
| **Homogamia** | algorytmy wzmacniają podobieństwo demograficzne |
| **Pierwsza rozmowa** | wyższe ryzyko rozbieżności między prezentacją online a rzeczywistością |

Finkel i współpracownicy (2012) w przeglądzie badań konkludują, że randkowanie online nie wykazuje przewagi nad metodami tradycyjnymi pod względem jakości i trwałości związku, choć ułatwia dostęp do partnerów.

---

## Krytyka podejścia ewolucyjnego

- **Problem odwrotnej inferencji** — nie można bezpośrednio wnioskować o funkcji adaptacyjnej z obserwowanych preferencji
- **Plastyczność kulturowa** — rozmiary efektów płciowych są modulowane przez poziom równości płci w danym społeczeństwie (*Zentner & Mitura, 2012* — w bardziej egalitarnych kulturach różnice maleją)
- **Agentywność jednostki** — podejście ewolucyjne może marginalizować rolę świadomych wyborów, narracji osobistych i konstruktów tożsamości

---

## Podsumowanie

| Perspektywa | Główne mechanizmy | Kluczowi badacze |
|---|---|---|
| Ewolucyjna | inwestycja rodzicielska, wskaźniki genetyczne | Trivers, Buss, Singh |
| Psychologiczna | podobieństwo-atrakcja, ekspozycja, filtrowanie | Byrne, Murstein, Zajonc |
| Socjokulturowa | homogamia, pole dostępnych partnerów | Kerckhoff, Zentner |
| Technologiczna | algorytmy, paradoks wyboru, atrakcyjność wizualna | Finkel, Schwartz |

---

## Literatura

- Buss, D. M. (1989). Sex differences in human mate preferences. *Behavioral and Brain Sciences, 12*(1), 1–14.
- Byrne, D. (1971). *The Attraction Paradigm*. Academic Press.
- Finkel, E. J., Eastwick, P. W., Karney, B. R., Reis, H. T., & Sprecher, S. (2012). Online dating: A critical analysis. *Psychological Science in the Public Interest, 13*(1), 3–66.
- Gangestad, S. W., & Simpson, J. A. (2000). The evolution of human mating. *American Psychologist, 55*(10), 1232–1238.
- Kerckhoff, A. C. (1974). The social context of interpersonal attraction. W: T. L. Huston (red.), *Foundations of Interpersonal Attraction*. Academic Press.
- Murstein, B. I. (1970). Stimulus-value-role: A theory of marital choice. *Journal of Marriage and the Family, 32*(3), 465–481.
- Schwartz, B. (2004). *The Paradox of Choice*. Harper Collins.
- Singh, D. (1993). Adaptive significance of female physical attractiveness. *Journal of Personality and Social Psychology, 65*(2), 293–307.
- Trivers, R. L. (1972). Parental investment and sexual selection. W: B. Campbell (red.), *Sexual Selection and the Descent of Man* (s. 136–179). Aldine.
- Zentner, M., & Mitura, K. (2012). Stepping out of the caveman's shadow. *Psychological Science, 23*(10), 1176–1185.
